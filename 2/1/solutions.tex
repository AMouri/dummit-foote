...
Consider first the last row of $A$ and the first column of $B$. Then $I_{n1} = 0 = (AB)_{n1} = \sum_{i=1}^n A_{ni}B_{i1} = A_{nn}B_{n1}$. Since $A_{nn} \neq 0$, then $B_{n1} = 0$. So, now consider the $i$th row of $A$ and the first column of $B$: suppose for some $i > 1$, that for all $k > i$ that $B_{k1} = 0$. Then, $0 = (AB)_{i1} = \sum_{j=1}^n A_{ij}B_{j1} = A_{ii}B_{i1} \rightarrow B_{i1} = 0$. Now consider the first row of $A$ and the first column of $B$: $1 = (AB)_{11} = \sum_{i=1}^n A_{1i}B_{i1} = A_{11}B_{11} \rightarrow B_{11} \neq 0$. 
